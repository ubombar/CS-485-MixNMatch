...
class DogsDataset(data.Dataset):
    ...
    def __init__(self,
        ...
        self.segmap_folder = join(self.root, 'Segmap')
        self.back_folder = join(self.root, 'Background')
    ...
    def __getitem__(self, index):
        image_name, _ = self._flat_breed_images[index]

        image_path = join(self.images_folder, image_name)
        segmap_path = join(self.segmap_folder, image_name)
        back_path = join(self.back_folder, image_name)

        image = Image.open(image_path).convert('RGB')
        segmap = Image.open(segmap_path).convert('RGB')
        back = Image.open(back_path).convert('RGB')
        
        latent = torch.normal(torch.zeros((4, 64)), torch.ones((4, 64)))

        z, b, p, c = latent[0], latent[1], latent[2], latent[3]

        return z, b, p, c, image, segmap, back
